...
export function useExpedientes() {
    ...
    const cargar = useCallback(async () => {
        ...
        setTechnicalDetail('');
        try {
            const res = await controlEscolarApi.expedientes(apiParams);
            setPayload(res?.data ?? null);
            setSelected([]);
        } catch (err) {
            setPayload(null);
            setError({
                message: sanitizeInstitutionalMessage(err?.message, 'No se pudo cargar expedientes.'),
                status: err?.status ?? 500,
            });
            setTechnicalDetail(
                err?.original?.response?.data
                    ? JSON.stringify(err.original.response.data, null, 2)
                    : String(err?.message ?? ''),
            );
        } finally {
            setLoading(false);
    ...
    return {
        ...
        documentos: payload?.documentos_requeridos ?? [],
        promedio: payload?.promedio_cumplimiento ?? 0,
        actividad: payload?.actividad_reciente ?? [],
