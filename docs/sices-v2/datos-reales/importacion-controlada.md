...
database/seeders/
  ...
  Support/           → Catálogo de permisos SICES

```



`DatabaseSeeder` ejecuta solo `InstitutionalBaseSeeder`. No existen seeders demo en el repositorio.
...
Los reportes están en `.gitignore` y no deben versionarse.
...
## Importación catálogos desde SISEES (MySQL legacy local)

Ver [mapeo-catalogos-sisees.md](./mapeo-catalogos-sisees.md).
...
Solo catálogos; el dump debe estar en `sisees_legacy` en localhost, no en la BD de producción.
...
## Patrones demo detectados (limpieza)
